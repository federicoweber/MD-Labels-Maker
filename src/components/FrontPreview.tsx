...
export default function FrontPreview({ data, size, update, onCover }: Props) {
  ...
  return (
    ...
    >
      <div
        className="group absolute top-0 left-0 cursor-pointer overflow-hidden"
      ...
      >
        {data.coverDataUrl && <img src={data.coverDataUrl} alt="" className="size-full object-cover" />}
        {/* Drop / click-to-replace overlay: always shown when empty, otherwise on
            hover or while a file is dragged over the page. */}
        <div
          className={`absolute inset-0 flex items-center justify-center transition-opacity ${
            data.coverDataUrl && !pageDragging ? 'opacity-0 group-hover:opacity-100' : 'opacity-100'
          }`}
          style={{
            background: data.coverDataUrl ? 'rgba(18,16,12,0.6)' : '#3f3d39',
            color: '#cfc9bd',
          }}
        >
          <svg
